param([string] $name, [string] $extensions, [string] $variables, [string] $solutionDir, [string] $projectDir, [string] $targetPath, [string] $configuration)

##Add errorhandling
trap {"Error in $name in $configuration build: $($_.InvocationInfo.PositionMessage)`r`n$($_.Exception.Message)"; exit 1; continue}

Set-Variable -Scope Global -Name "solutionDir"   -Value $solutionDir
Set-Variable -Scope Global -Name "projectDir"    -Value $projectDir
Set-Variable -Scope Global -Name "targetPath"    -Value $targetPath
Set-Variable -Scope Global -Name "configuration" -Value $configuration


function LoadExtensions($s){
	Write-Host "LoadExtensions $s"
	if($s -eq $null) {
		return
	}
    $arr = $s -split ","
...
function LoadVariables($s){
	Write-Host "LoadVariables $s"
	if($s -eq $null) {
		return
	}
    $arr = $s -split ","
    foreach($f in $arr) {
		...
		$kv= $f -split "="
		#Write-Host "Set-Variable -name $($kv[0]) -Value $($kv[1])"
